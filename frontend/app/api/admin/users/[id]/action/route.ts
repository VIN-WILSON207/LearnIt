...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const action = body.action; // suspend | delete | restore

    const backendRes = await fetch(`${BACKEND_BASE}/api/admin/users/${id}/${action}`, {
      method: 'POST',
      headers: {
    ...
    console.error('Error proxying admin user action:', error);
