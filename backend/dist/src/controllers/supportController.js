...
exports.updateMessageStatus = exports.addResponse = exports.getAllMessages = exports.getMessage = exports.getUserMessages = exports.createMessage = void 0;
...
const createMessage = async (req, res) => {
    try {
        ...
        const supportMessage = await prisma_1.default.supportMessage.create({
            data: {
                userId,
                subject,
                message,
        ...
        if (supportMessage.user?.email) {
            await sendEmail(supportMessage.user.email, `Support Request Received: ${subject}`, userEmailHtml);
        }
        ...
        `;
        await sendEmail(SUPPORT_EMAIL, `New Support Request: ${subject}`, adminEmailHtml);
...
const getUserMessages = async (req, res) => {
    try {
        ...
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        const messages = await prisma_1.default.supportMessage.findMany({
            where: { userId },
            include: {
...
const getMessage = async (req, res) => {
    try {
        ...
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        const message = await prisma_1.default.supportMessage.findUnique({
            where: { id },
            include: {
                user: { select: { id: true, name: true, email: true } },
                responses: {
...
const addResponse = async (req, res) => {
    try {
        ...
        const supportResponse = await prisma_1.default.supportResponse.create({
            data: {
                messageId: id,
                adminId,
                response,
        ...
        if (message.user?.email) {
            await sendEmail(message.user.email, `Response to Your Support Request: ${message.subject}`, userEmailHtml);
        }
...
const updateMessageStatus = async (req, res) => {
    try {
        ...
        if (req.user?.role !== 'ADMIN')
            return res.status(403).json({ error: 'Forbidden' });
        const message = await prisma_1.default.supportMessage.update({
            where: { id },
            data: { status },
            include: {
                user: { select: { name: true, email: true } },
